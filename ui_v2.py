...
def apply_theme():
    """Apply the shared StockDash theme."""
    st.markdown(
        """
<style>
:root {
...
section[data-testid="stSidebar"] { background:linear-gradient(180deg,var(--navy) 0%,#0E1B3D 100%); border-right:0; }
...
[data-testid="stSidebar"] .stRadio div[role="radiogroup"] label { color:#B9C5DE; border-radius:11px; padding:.65rem .75rem; transition:background .15s ease,color .15s ease; }
[data-testid="stSidebar"] .stRadio div[role="radiogroup"] label:hover { background:rgba(255,255,255,.07); color:#FFFFFF; }
[data-testid="stSidebar"] .stRadio div[role="radiogroup"] label:has(input:checked) { background:rgba(37,99,235,.22); color:#FFFFFF; font-weight:750; box-shadow:inset 3px 0 var(--blue); }
section[data-testid="stSidebar"] p, section[data-testid="stSidebar"] label, section[data-testid="stSidebar"] [data-testid="stCaptionContainer"] { color:#B9C5DE; }
h1,h2,h3,h4 { color:var(--text); letter-spacing:-.035em; }
h1 { font-weight:800; } h2,h3 { font-weight:760; }
p,li { line-height:1.62; }
[data-testid="stCaptionContainer"] { color:var(--muted); }
[data-testid="stMetric"] { background:var(--surface); border:1px solid var(--line); border-radius:14px; padding:15px 17px; box-shadow:0 8px 26px rgba(15,23,42,.035); }
[data-testid="stMetricLabel"] { color:var(--muted); }
[data-testid="stMetricValue"] { color:var(--text); font-weight:780; }
[data-testid="stVerticalBlockBorderWrapper"] { border-color:var(--line) !important; border-radius:15px !important; background:var(--surface); box-shadow:0 8px 26px rgba(15,23,42,.035); }
.stButton > button,.stFormSubmitButton > button { border-radius:10px; min-height:2.65rem; font-weight:700; }
.stButton > button[kind="primary"],.stFormSubmitButton > button[kind="primary"] { background:var(--blue); border-color:var(--blue); }
.stTextInput input,.stTextArea textarea,.stSelectbox div[data-baseweb="select"] > div { border-radius:10px !important; }
...
.planx-brand-mark { width:38px;height:38px;border-radius:11px;display:flex;align-items:center;justify-content:center;background:linear-gradient(145deg,#3B82F6,#1D4ED8);color:#fff;font-size:20px;font-weight:850;box-shadow:0 7px 18px rgba(37,99,235,.28); }
...
.planx-hero { background:linear-gradient(135deg,#FFFFFF 0%,#F9FBFF 55%,#EEF4FF 100%); border:1px solid #E2E8F0; border-radius:20px; padding:25px 28px; margin-bottom:17px; box-shadow:0 12px 34px rgba(15,23,42,.045); }
...
.planx-card { background:#fff; border:1px solid var(--line); border-radius:14px; padding:18px 19px; min-height:116px; box-shadow:0 7px 22px rgba(15,23,42,.03); }
...
/* StockDash dashboard */
.stockdash-topbar { position:relative; overflow:hidden; display:flex; align-items:flex-end; justify-content:space-between; gap:24px; padding:30px 32px; margin:0 0 22px; border-radius:24px; background:linear-gradient(120deg,#07142f 0%,#102657 58%,#1d4ed8 100%); box-shadow:0 18px 45px rgba(15,23,42,.14); }
.stockdash-topbar:after { content:""; position:absolute; width:300px; height:300px; right:-90px; top:-170px; border:1px solid rgba(255,255,255,.16); border-radius:50%; box-shadow:0 0 0 32px rgba(255,255,255,.035),0 0 0 64px rgba(255,255,255,.025); }
...
.stockdash-search { position:relative; z-index:1; min-width:280px; background:rgba(255,255,255,.10); border:1px solid rgba(255,255,255,.16); border-radius:12px; padding:11px 14px; color:#D9E4FA; font-size:12px; }
...
.stockdash-positive { color:#16A34A !important; }
.stockdash-negative { color:#DC2626 !important; }
...
.stockdash-stock-name { font-weight:750; font-size:13px; }
.stockdash-stock-code { font-size:10px; color:#94A3B8; margin-left:6px; }
.stockdash-stock-price { text-align:right; font-weight:750; font-size:13px; }
.stockdash-stock-change { text-align:right; font-size:12px; }
...
@media(max-width:900px) {
  ...
  .stockdash-market { grid-template-columns:repeat(2,1fr); }
  .stockdash-search { min-width:0; width:220px; }
}
@media(max-width:640px) {
  .block-container { padding-top:1.1rem; }
  .planx-card { min-height:100px; padding:14px; }
  .planx-card-value { font-size:22px; }
  .stockdash-topbar { align-items:flex-start; flex-direction:column; }
  .stockdash-search { width:100%; min-width:0; }
  .stockdash-market { grid-template-columns:1fr 1fr; }
...
<div class="planx-brand">
  ...
  <div>
    ...
    <div class="planx-brand-sub">INVESTMENT DASHBOARD</div>
  </div>
</div>
""",
        unsafe_allow_html=True,
